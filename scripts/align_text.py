    ...
    if len(edits) < 1:
        ...
        for e in edits:
            ...
            if ((s and (ispunct(s) or s.orth_[0].isupper())) or (t and (ispunct(t) or t.orth_[0].isupper()))) and \
                ...
                #				print("RULE 2")
                return get_edits(source, target, edits[:i]) + merge_edits(edits[i:j + 1]) + get_edits(source, target,
                                                                                                      edits[j + 1:])
            # Keep all T separate.
...
def getAutoAlignedEdits(orig, cor, spacy, args):
    ...
    if args.lev:
        alignments = DL.WagnerFischer(orig_toks, cor_toks, orig, cor, substitution=levSubstitution,
                                      transposition=levTransposition)
    ...
    elif args.merge == "all-split":
        edits = get_edits_split(get_opcodes(alignment))
    elif args.merge == "all-merge":
        edits = get_edits_group_all(get_opcodes(alignment))
    elif args.merge == "all-equal":
        edits = get_edits_group_type(get_opcodes(alignment))
    proc_edits = []
    for edit in edits:
        orig_start = edit[1]
        orig_end = edit[2]
...
def get_alignment(orig, cor, spacy):
    """
    ...
    # Otherwise, use linguistically enhanced Damerau-Levenshtein
        ...
        alignments = DL.WagnerFischer(orig_toks, cor_toks, orig, cor, substitution=token_substitution)
    # Get the alignment with the highest score. There is usually only 1 best in DL due to custom costs.
    alignment = next(alignments.alignments(False))
    if args_merge == "rules":
        edits = get_edits(orig, cor, get_opcodes(alignment))
    elif args_merge == "all-split":
        edits = get_edits_split(get_opcodes(alignment))
    elif args_merge == "all-merge":
        edits = get_edits_group_all(get_opcodes(alignment))
    elif args_merge == "all-equal":
        edits = get_edits_group_type(get_opcodes(alignment))
...
#get_errant_alignment(incorrect_file=incorrect_file, correct_file=correct_file, alignment_file=alignment_file)
